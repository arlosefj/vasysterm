...
#解析码流为图片
def capture(camcode,camnum):#campreset resevered
		try:
            time_local = time.localtime()
            #转换成新的时间格式(2016-05-09 18:59:20)
			strtime = time.strftime("%Y%m%d%H%M%S",time_local)
			#获取预置位编号和时间，给图片取名字 名字根据时间等给定
			picname = camcode + "%24" + camnum + "_" + strtime + ".jpg"
			str = "./test 2 \"rtsp://10.13.7.11:9090/dss/monitor/param?cameraid="+camcode+"%24"+camnum+"&substream=1\" "+picname
			os.system(str)
		except:
