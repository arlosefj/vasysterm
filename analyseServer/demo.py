...
def parse_args():
    ...
    parser.add_argument('--images', dest='images', type=str, default='./data/demo/IMG_6194.jpg',
...
def run_images(args,ctx,pic_str):
    # parse image list
    ...
    # run detection
    detector.detect_and_visualize(image_list,pic_str, args.dir, args.extension,
                                  class_names, args.thresh, args.show_timer)


def run_coordinate(args,ctx):
    # wjh
    # return coor是一张图的所有店铺坐标（列表）
    image_list = [i.strip() for i in args.images.split(',')]
    assert len(image_list) > 0, "No valid image specified to detect"
    ...
    detector = get_detector(network, prefix, args.epoch,
                            ...
                            (args.mean_r, args.mean_g, args.mean_b),
                            ctx, len(class_names), args.nms_thresh, args.force_nms)
    coor = detector.detect_coordinate(image_list, args.dir, args.extension,
                                  class_names, args.thresh, args.show_timer)
    return coor
...
def get_coordinate(img):
    ...
    coor_js = run_coordinate(args, ctx)
    return coor_js
...
def main():
    ...
    if args.camera:
        ...
        for d in a:
            str = c+d+b
            args.images = str
            run_images(args, ctx, d)
    '''
    get_coordinate("./myself/timg.jpg")
    return 0
